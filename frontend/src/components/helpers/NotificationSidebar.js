...
const NotificationSidebar = ({ isNotificationsVisible, closePanel }) => {
  ...
  console.log("is: ", isNotificationsVisible);

  useEffect(() => {
    const getNotifications = async () => {
      try {
        const token = localStorage.getItem("token");
        const response = await axios.get(`http://localhost:3001/api/noti/getNoti`, 
        {
  ...
  const handleNotificationClick = async (noti) => {
    ...
    else if (noti.type === 'Follow Request') {

    }
  };

  const handleFollowRequest = (noti, action) => {
                  ...
                  {notif.type === "Follow Request" && (
                    <div className="flex gap-2 mt-2">
                      ...
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          handleFollowRequest(notif, "delete");
                        }}
